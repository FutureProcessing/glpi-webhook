...
class PluginFpwebhookQueue extends CommonDBTM
{
   ...
   private const ACCEPTED_RESPONSES = [200, 201, 202, 203, 204, 205, 302, 303, 307]; // includes temporary redirects
    ...
    * @return bool
    */
   static public function queueMessage(int $subscription_id, int $content_id): bool
   {
      global $DB;
   ...
   public static function cronSendResponses(): int
   {
      ...
      while ($message = $messages->next()) {
         ...
         if (in_array($status, self::KILLING_RESPONSES)) {
            $result = $result && PluginFpwebhookSubscription::unsubscribe(
   ...
   public static function cronCleanQueue(): int
   {
      ...
      while ($message = $messages->next()) {
         if ($message['attempts'] >= ($my_config['max_attempts_per_message'] ?? 3)) {
            // If the delivery was attempted several times and still failed, mark this as failure of the receiver
            $result = $result && PluginFpwebhookSubscription::addFailure($message['subscription_id']);
         }
   ...
   private static function fetchMessagesToSend(): DBmysqlIterator
   {
      ...
      return $DB->request(
         [
            ...
            'WHERE' => [
               'attempts < ' . ($my_config['max_attempts_per_message'] ?? 3),
            ...
            'LIMIT' => ($my_config['max_messages_per_tick'] ?? 3),
         ]
      );
   }
   ...
   private static function fetchMessagesToRemove(): DBmysqlIterator
   {
      ...
      return $DB->request(
         [
            ...
            'WHERE' => [
               'OR' => [
                  'attempts >= ' . ($my_config['max_attempts_per_message'] ?? 3), // too many attempts
                  'is_active = 0',
                  'is_deleted = 1',
